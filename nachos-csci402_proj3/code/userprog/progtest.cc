...
StartProcess(char *filename)
{
    ...
    // Fill the information of thread
    currentThread->space = space;
    currentThread->threadinfo->threadLock->Acquire();
    currentThread->threadinfo->parentThread = currentThread;
    currentThread->threadinfo->threadLock->Release();

    currentThread->space->fileTable.Put(executable);
    ...
    proc->processId = pid;
    proc->parentAddrSpace->processId = pid;
    processTableLock -> Release();
    currentThread->space->processId = pid;
    threadTableLock -> Acquire();
    int tid = threadTable -> Put((void*)currentThread);
    if (tid<0) {
        printf("Exceeding the max thread Amount.\n");
        threadTableLock -> Release();
        return ;
    }
    currentThread->threadinfo->tid = tid;
    currentThread->threadinfo->pid = pid;
    threadTableLock -> Release();
