...
AddrSpace::AddrSpace(OpenFile *executable) : fileTable(MaxOpenFiles) {
    ...
    int memStartPage = mem->FindPages(numPages);
    if (memStartPage == -1) {
...
AddrSpace::~AddrSpace()
{
    ...
    for (unsigned int i=0; i<numPages; i++) {
        pid = pageTable[i].physicalPage;
        mem->ClearPages(pid, 1);
    }
    delete pageTable;
}
...
int AddrSpace::AllocateNewStack() {
    ...
    int memStartPage = mem->FindPages(addpages);
    if (memStartPage == -1) {
        DEBUG('a', "No Enough Main Memory in Nachos.\n");
        return -1;
    }
...
int AddrSpace::DeallocateStack() {
    ...
    RestoreState();
    mem->ClearPages(memStartPage, pages);
    return 1;
}
