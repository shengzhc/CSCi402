...
void t1_t0()
{
    Print("\nThe t1_t0 test is for negative index of lock.\n", sizeof("\nThe t1_t0 test is for negative index of lock.\n")/sizeof(char), 0, 0);
    AcquireLock(-20);
    Exit(0);
}
void t1_t1(){
...
void t1_t2(){
    ...
    if(AcquireLock(lock1))
        ...
        Print("t1_t2 hasnot acquired lock1 successfully.\n", sizeof("t1_t2 hasnot acquired lock1 successfully.\n")/sizeof(char), 0, 0);
    if(ReleaseLock(lock1))
...
void t3_t2(){
    if(AcquireLock(lock1))
        Print("t3_t2 has acquired lock1.\n", sizeof("t2_t1 has acquired lock1.\n")/sizeof(char), 0, 0);
